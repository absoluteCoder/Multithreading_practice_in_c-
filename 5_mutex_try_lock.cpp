...
std::mutex m;
void addMoney(){
  // other tx will not be blocked
 for(int i = 0; i < 1000; i++){
    if(m.try_lock()){
      // if we comment above line and use m.lock() then other thread will be blocked to use shared variable hence it will 2000
      money++;
      m.unlock();
    }
 }
...
int main(){
  ...
  t1.join();
  t2.join();
  //779  [0, 2000]came back after processing
